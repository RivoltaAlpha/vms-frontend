...
export const BookingForm = () => {
  ...
  const { location_id, vehicle_id } = useParams<{ location_id: string; vehicle_id: string }>();

  const navigate = useNavigate();

  const [formData, setFormData] = useState<BookingFormData>({
    user_id: user?.user_id || 0,
    vehicle_id: vehicle_id ? parseInt(vehicle_id) : 0,
  ...
  useEffect(() => {
    const fetchVehicle = async () => {
      try {
        const response = await fetch(`/vehicle/${vehicle_id}`);
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
      ...
      <h2 className="text-2xl font-bold mb-5">
        {vehicle ? `Book ${vehicle.vehicleSpec.manufacturer} ${vehicle.vehicleSpec.model}` : 'Still Loading vehicle...'}
      </h2>
